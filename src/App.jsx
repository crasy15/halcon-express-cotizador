...
export default function App() {
  ...
  const usarGPS = (tipo) => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        try {
            const res = await fetch(`https://halcon-express-cotizador.onrender.com/reverse-geocode?lat=${coords[0]}&lon=${coords[1]}`);
  ...
  const recalcularRuta = async (p1, p2) => {
    try {
        const res = await fetch("https://halcon-express-cotizador.onrender.com/route", {
  ...
  const calcularPorInput = async () => {
    try {
      ...
      const buscarCoords = async (texto) => {
        const res = await fetch(`https://halcon-express-cotizador.onrender.com/geocode?q=${encodeURIComponent(texto)}`);
        const data = await res.json();
  ...
  const obtenerDireccionDeCoords = async (lat, lon) => {
    try {
        const res = await fetch(`https://halcon-express-cotizador.onrender.com/reverse-geocode?lat=${lat}&lon=${lon}`);
        const data = await res.json();
